...
if (!prod && !vaultPath) {
	console.warn("⚠️  OBSIDIAN_VAULT_PATH not set. Plugin will only build locally.");
...
if (vaultPluginDir && !prod) {
	try {
		...
		console.log(`✓ Building to vault: ${vaultPluginDir}`);
	} catch (err) {
		console.error(`❌ Failed to create vault plugin directory: ${err.message}`);
		process.exit(1);
	}
}
			...
			setup(build) {
				build.onEnd(async (result) => {
					...
					try {
						...
						console.log(`✓ Copied to vault at ${new Date().toLocaleTimeString()}`);
					} catch (err) {
